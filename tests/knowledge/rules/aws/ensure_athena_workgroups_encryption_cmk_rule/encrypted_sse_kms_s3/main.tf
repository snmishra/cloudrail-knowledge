...
resource "aws_s3_bucket" "cloudrail_anthena_bucket" {
  bucket = "cloudrail-wg-encrypted-sse-kms-s3"
  acl    = "private"
}
...
resource "aws_athena_workgroup" "cloudrail_wg" {
  name = "cloudrail-wg-encrypted-sse-kms-s3"
  ...
  configuration {
    ...
    result_configuration {
      ...
      encryption_configuration {
        ...
        kms_key_arn       = data.aws_kms_alias.s3.arn # "arn:aws:kms:<REGION>:<ACCOUNT-ID>:alias/aws/s3"
      }
    }
  }
}
