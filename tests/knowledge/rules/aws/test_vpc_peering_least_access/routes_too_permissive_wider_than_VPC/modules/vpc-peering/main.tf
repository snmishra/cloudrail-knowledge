...
resource "aws_vpc_peering_connection_options" "this" {
  ...
  count = 1 # Replaced this: local.this_region == local.peer_region ? 1 : 0
...
resource "aws_route" "this_routes_region" {
  provider                  = "aws.this"
  count                     = 1 # Replaced this: length(data.aws_route_tables.this_vpc_rts.ids)
...
resource "aws_route" "peer_routes_region" {
  provider                  = "aws.peer"
  count                     = 1 # Replaced this: length(data.aws_route_tables.peer_vpc_rts.ids)
