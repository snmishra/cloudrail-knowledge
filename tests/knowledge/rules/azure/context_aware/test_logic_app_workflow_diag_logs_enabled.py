...
class TestLogicAppWorkflowDiagnosticLogsEnabledRule(unittest.TestCase):
    def setUp(self):
        self.rule = LogicAppWorkflowDiagnosticLogsEnabledRule()
    ...
    def test_states(self, unused_name: str, monitor_diagnostic_settings: AzureMonitorDiagnosticSetting, should_alert: bool):
        # Arrange
        logic_app_workflow: AzureLogicAppWorkflow = create_empty_entity(AzureLogicAppWorkflow)
        logic_app_workflow.name = 'tmp-name'
        logic_app_workflow.set_id('diag_monitor_setting_key')
        logic_app_workflow.with_aliases(logic_app_workflow.get_id())
        logic_app_workflow.monitor_diagnostic_settings = [monitor_diagnostic_settings] if monitor_diagnostic_settings else []

        context = AzureEnvironmentContext(logic_app_workflows=AliasesDict(logic_app_workflow),
