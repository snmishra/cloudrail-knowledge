...
class TestDataLakeAnalyticsDiagnosticLogsEnabledRule(unittest.TestCase):
    ...
    def test_states(self, unused_name: str, monitor_diagnostic_settings: AzureMonitorDiagnosticSetting, should_alert: bool):
        # Arrange
        data_lak_analyitics_account: AzureDataLakeAnalyticsAccount = create_empty_entity(AzureDataLakeAnalyticsAccount)
        data_lak_analyitics_account.name = 'tmp-name'
        data_lak_analyitics_account.set_id('diag_monitor_setting_key')
        data_lak_analyitics_account.with_aliases(data_lak_analyitics_account.get_id())
        data_lak_analyitics_account.monitor_diagnostic_settings = [monitor_diagnostic_settings] if monitor_diagnostic_settings else []

        context = AzureEnvironmentContext(data_lake_analytics_accounts=AliasesDict(data_lak_analyitics_account),
