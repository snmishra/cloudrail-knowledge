...
class TestVmssExtension(AzureContextTest):
    ...
    def test_basic(self, ctx: AzureEnvironmentContext):
        vmss_ext = next((ext for ext in ctx.vmss_extentions if ext.name == 'example'))
        ...
        self.assertTrue(vmss_ext.virtual_machine_scale_set_id in
        ...
        self.assertTrue(vmss.extension_config)
        self.assertEqual(vmss.extension_config.publisher, 'Microsoft.Azure.Diagnostics')

    @context(module_path="basic_nested_block")
    def test_basic_nested_block(self, ctx: AzureEnvironmentContext):
        vmss_ext = next((ext for ext in ctx.vmss_extentions if ext.name == 'MSILinuxExtension'))
        self.assertIsNotNone(vmss_ext)
        ...
        self.assertIsNotNone(vmss)
        self.assertTrue(vmss.extension_config)
        self.assertEqual(vmss.extension_config.publisher, 'Microsoft.ManagedIdentity')
