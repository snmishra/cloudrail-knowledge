...
class BaseRulesMetadataStore:
    ...
    def merge(self, rule_metadata_store: 'BaseRulesMetadataStore'):
    ...
    def _fill_templates(rules, templates):
        for rule in rules:
            if BaseRulesMetadataStore._is_template(rule.get(NAME)):
                rule[NAME] = BaseRulesMetadataStore._fill_template(rule[NAME], templates)
            if BaseRulesMetadataStore._is_template(rule.get(DESCRIPTION)):
                rule[DESCRIPTION] = BaseRulesMetadataStore._fill_template(rule[DESCRIPTION], templates)
            if BaseRulesMetadataStore._is_template(rule.get(LOGIC)):
                rule[LOGIC] = BaseRulesMetadataStore._fill_template(rule[LOGIC], templates)
            if BaseRulesMetadataStore._is_template(rule.get(IAC_REMEDIATION_STEPS)):
                rule[IAC_REMEDIATION_STEPS] = BaseRulesMetadataStore._fill_template(rule[IAC_REMEDIATION_STEPS], templates)
            if BaseRulesMetadataStore._is_template(rule.get(CONSOLE_REMEDIATION_STEPS)):
                rule[CONSOLE_REMEDIATION_STEPS] = BaseRulesMetadataStore._fill_template(rule[CONSOLE_REMEDIATION_STEPS], templates)
        return rules
