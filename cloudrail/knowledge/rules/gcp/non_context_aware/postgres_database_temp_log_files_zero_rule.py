...
class PostgresDatabaseTempLogFilesZeroRule(BaseDatabaseFlagOnRule):

    def get_id(self) -> str:
        return 'non_car_cloud_sql_log_temp_files_zero'

    def execute(self, env_context: GcpEnvironmentContext, parameters: Dict[ParameterType, any]) -> List[Issue]:
        issues: List[Issue] = []
        for sql_db in env_context.sql_database_instances:
            if self.is_version_contains(sql_db, 'POSTGRES') and \
                    not self.is_flag_mode(sql_db, 'log_temp_files', '0'):
                issues.append(
                    Issue(
                        f"The Google Cloud {sql_db.get_type()} `{sql_db.get_friendly_name()}` has database"
                        f" flag `log_temp_files` set other than 0.",
                        sql_db,
                        sql_db))
        return issues
