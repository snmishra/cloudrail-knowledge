...
class TestEnsureSecurityGroupRuleIncludeDescriptionRule(AwsBaseRuleTest):
    ...
    def test_no_description_for_rules(self):
        ...
        self.assertEqual("The Security group `aws_security_group.default` "
                         ...
                         " for ports 443:443 using protocol TCP`", rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'Security group')
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'examplerulename')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group rule')
        self.assertEqual(rule_result.issue_items[0].violating.iac_entity_id, 'aws_security_group.default')
    ...
    def test_multiple_items_for_rules(self):
        ...
        for item in rule_result.issue_items:
            ...
            self.assertEqual(item.exposed.type, 'Security group')
            self.assertEqual(item.violating.type, 'Security group rule')
    ...
    def test_no_description_for_security_group(self):
        ...
        self.assertEqual(rule_result.issue_items[0].violating.name, 'examplerulename')
    ...
    def test_implicit_rule_without_description(self):
        ...
        self.assertTrue('The Security group `aws_security_group.instance` does not have a description for the `egress rule' in
                        rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'Security group')
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'some-sg-name')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group rule')
