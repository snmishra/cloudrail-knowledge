...
class TestPublicAccessSecurityGroupsPortRule(AwsBaseRuleTest):
    ...
    def test_port_22_allowed_from_internet_to_ec2_explicit(self):
        ...
        self.assertTrue("Instance uses security group `aws_security_group.sg`."
                        " `aws_security_group.sg` allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.iac_entity_id, 'aws_instance.test')
    ...
    def test_port_22_allowed_from_internet_to_ec2_using_tf_complete_vpc_module(self):
        rule_result = self.run_test_case('port_22_allowed_from_internet_to_ec2_using_tf_complete_vpc_module', always_use_cache_on_jenkins=True)
        self.assertIsNotNone(rule_result)
        self.assertTrue("Instance uses security group `aws_security_group.sg`."
                        " `aws_security_group.sg` allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.iac_entity_id, 'aws_instance.test')
    ...
    def test_port_22_allowed_from_internet_but_instance_on_private_subnet(self):
        self.run_test_case('port_22_allowed_from_internet_but_instance_on_private_subnet', False, always_use_cache_on_jenkins=True)

    def test_port_22_allowed_from_internet_to_ec2_using_tf_ssh_module(self):
        rule_result = self.run_test_case('port_22_allowed_from_internet_to_ec2_using_tf_ssh_module', True, always_use_cache_on_jenkins=True)
        self.assertIsNotNone(rule_result)
        self.assertTrue("allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.iac_entity_id, 'aws_instance.test')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'EC2 Instance')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group')

    def test_port_22_allowed_from_internet_to_ec2_using_tf_instance_module(self):
        rule_result = self.run_test_case('port_22_allowed_from_internet_to_ec2_using_tf_instance_module', True, always_use_cache_on_jenkins=True)
        self.assertIsNotNone(rule_result)
        self.assertTrue("allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'PublicAccessSecurityGroupsPort test - use case 4')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'EC2 Instance')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group')
    ...
    def test_port_22_allowed_from_internet_to_load_balancer_explicit(self):
        ...
        self.assertIsNotNone(rule_result)
        self.assertTrue("allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'aws_lb.test.name')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'Load Balancer')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group')
    ...
    def test_atlantis_only(self):
        self.run_test_case('atlantis_only', False, always_use_cache_on_jenkins=True)

    def test_bastion_server(self):
        rule_result = self.run_test_case('bastion_server', True, always_use_cache_on_jenkins=True)
        self.assertIsNotNone(rule_result)
        self.assertTrue("allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'test-dev-bastion')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'EC2 Instance')
        self.assertEqual(rule_result.issue_items[0].violating.name, 'test-dev-bastion')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group')
    ...
    def test_auto_scaling_group_public_ip_exposure(self):
        ...
        self.assertIsNotNone(rule_result)
        self.assertTrue("allows port `22`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'test-autoscaling-group-pseudo-instance-subnet-public-subnet')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'EC2 Instance')
        self.assertEqual(rule_result.issue_items[0].violating.name, 'aws_security_group.allow-ssh.name')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Security group')
    ...
    def test_port_22_allowed_from_internet_to_ec2_instances_from_launch_config(self):
        ...
        for issue_item in rule_result.issue_items:
            ...
            self.assertEqual(issue_item.violating.type, 'Security group')
