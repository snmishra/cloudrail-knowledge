...
class TestPublicAccessDbRdsRule(AwsBaseRuleTest):
    ...
    def test_aurora_vpc_controlled_public(self):
        ...
        self.assertTrue("RDS Instance uses subnet(s) `aws_security_group.nondefault`." in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.iac_entity_id, 'aws_rds_cluster.test')
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'RDS DB cluster')
        self.assertEqual(rule_result.issue_items[0].violating.name, 'aws_security_group.nondefault.name')
    ...
    def test_individual_vpc_controlled_public(self):
        ...
        self.assertTrue("RDS Instance uses the subnets `aws_subnet.nondefault_1, aws_subnet.nondefault_2`."
                        " RDS Instance is reachable" in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.iac_entity_id, 'aws_db_instance.test')
