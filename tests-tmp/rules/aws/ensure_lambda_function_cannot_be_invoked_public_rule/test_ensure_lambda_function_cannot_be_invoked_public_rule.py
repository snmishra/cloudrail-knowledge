...
class TestEnsureLambdaFunctionCannotBeInvokedPublicRule(AwsBaseRuleTest):
    ...
    def test_link_with_arn(self):
        self.run_test_case('link_with_arn', False, always_use_cache_on_jenkins=True)

    def test_link_with_function_name(self):
        self.run_test_case('link_with_function_name', False, always_use_cache_on_jenkins=True)

    def test_link_with_partial_arn(self):
        self.run_test_case('link_with_partial_arn', False, always_use_cache_on_jenkins=True)

    def test_link_with_using_qualifier(self):
        self.run_test_case('link_with_using_qualifier', False, always_use_cache_on_jenkins=True)

    def test_link_with_using_qualifier_public_lambda(self):
        self.run_test_case('link_with_using_qualifier_public_lambda', True, always_use_cache_on_jenkins=True)
