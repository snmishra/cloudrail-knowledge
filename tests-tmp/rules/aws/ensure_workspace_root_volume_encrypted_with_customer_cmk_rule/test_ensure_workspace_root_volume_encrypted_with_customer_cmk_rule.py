...
class TestEnsureWorkspaceRootVolumeEncryptionCmkRule(AwsBaseRuleTest):
    ...
    def test_aws_cmk_root_volume_encryption(self):
        self.run_test_case('aws_cmk_root_volume_encryption', True, always_use_cache_on_jenkins=True)

    def test_customer_cmk_root_volume_created(self):
        self.run_test_case('customer_cmk_root_volume_created', False, always_use_cache_on_jenkins=True)

    def test_customer_cmk_root_volume_existing(self):
        self.run_test_case('customer_cmk_root_volume_existing', False, always_use_cache_on_jenkins=True)
