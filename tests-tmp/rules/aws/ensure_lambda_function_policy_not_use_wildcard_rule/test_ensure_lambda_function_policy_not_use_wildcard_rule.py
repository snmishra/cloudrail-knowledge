...
class TestEnsureLambdaFunctionPolicyNotUseWildcard(AwsBaseRuleTest):
    ...
    def test_not_secure_policy(self):
        ...
        self.assertTrue('is using wildcard action `lambda:*`, without any condition' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'Lambda Function')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'Lambda Policy')
