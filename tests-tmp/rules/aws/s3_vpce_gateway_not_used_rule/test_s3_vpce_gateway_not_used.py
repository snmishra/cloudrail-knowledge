...
class TestS3VpceGatewayNotUsed(AwsBaseRuleTest):
    ...
    def test_vpc_instance_has_direct_s3_service_connection(self):  # igw + bucket + eni
        self.run_test_case('vpc_instance_has_direct_s3_service_connection', True, always_use_cache_on_jenkins=True)
    ...
    def test_vpc_has_s3_vpce_gw_and_public_connection(self):  # igw + bucket + eni + vpce
        self.run_test_case('vpc_has_s3_vpce_gw_and_public_connection', False, always_use_cache_on_jenkins=True)

    def test_vpc_has_only_s3_vpce_gw_connection(self):  # igw + bucket + eni + vpce + natgw (private)
        self.run_test_case('vpc_has_only_s3_vpce_gw_connection', False, always_use_cache_on_jenkins=True)
