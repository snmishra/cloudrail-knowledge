...
class TestEnsureWorkspaceUserVolumeEncryptedAtRestWithCustomerManagedCmkRule(AwsBaseRuleTest):
    ...
    def test_user_volume_encrypted_with_aws_managed_cmk(self):
        self.run_test_case('user_volume_encrypted_with_aws_managed_cmk', True, always_use_cache_on_jenkins=True)

    def test_user_volume_encrypted_with_customer_managed_cmk_creating_key(self):
        self.run_test_case('user_volume_encrypted_with_customer_managed_cmk_creating_key', False, always_use_cache_on_jenkins=True)

    def test_user_volume_encrypted_with_customer_managed_cmk_existing_key(self):
        self.run_test_case('user_volume_encrypted_with_customer_managed_cmk_existing_key', False, always_use_cache_on_jenkins=True)
