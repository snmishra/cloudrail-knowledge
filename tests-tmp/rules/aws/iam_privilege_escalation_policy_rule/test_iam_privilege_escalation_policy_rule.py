...
class TestIamPrivilegeEscalationPolicyRule(AwsBaseRuleTest):
    ...
    def test_iam_all_resources(self):  # actions: "iam: AttachUser*", "ec2:createroute"
        ...
        self.assertTrue('granting the problematic permissions' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].violating.iac_resource_metadata.iac_entity_id, 'aws_iam_role_policy.policy')
        self.assertEqual(rule_result.issue_items[0].exposed.iac_resource_metadata.iac_entity_id, 'aws_iam_role_policy.policy')
    ...
    def test_escalation_permissions_not_denied_resource_mismatch(self):  # actions: "iam:CreateAccessKey", resource: "*"
        rule_result = self.run_test_case('escalation-permissions-not-denied-resource-mismatch', True, always_use_cache_on_jenkins=True)
        self.assertIsNotNone(rule_result)
        self.assertTrue('granting the problematic permissions `{\'iam:CreateAccessKey\'}`' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'allow-policy')
        self.assertEqual(rule_result.issue_items[0].violating.iac_resource_metadata.iac_entity_id, 'aws_iam_user_policy.allow-policy')
    ...
    def test_combo_escalation_permissions(self):  # actions: "iam: passrole", "lambda: createfunction", "lambda: invokefunc*"
        ...
        self.assertIsNotNone(rule_result)
        self.assertTrue(all(item in rule_result.issue_items[0].evidence
                            for item in ("lambda: invokefunc*", "iam: passrole", "lambda: createfunction", "conjunction")))
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'allow-policy-1')
        self.assertEqual(rule_result.issue_items[0].violating.iac_resource_metadata.iac_entity_id, 'aws_iam_role_policy.allow-policy-1')
    ...
    def test_iam_all_actions(self):  # actions: "*"
        ...
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'allow-all-actions')
        self.assertEqual(rule_result.issue_items[0].violating.iac_resource_metadata.iac_entity_id, 'aws_iam_role.role')
    ...
    def test_inline_policy_all_actions(self):
        ...
        self.assertTrue("granting the problematic permissions `{\'*\'}` which can allow for privilege escalation"
                        in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.name, 'ec2-describe-policy')
        self.assertEqual(rule_result.issue_items[0].violating.iac_resource_metadata.iac_entity_id, 'aws_iam_role.ec2-role')
