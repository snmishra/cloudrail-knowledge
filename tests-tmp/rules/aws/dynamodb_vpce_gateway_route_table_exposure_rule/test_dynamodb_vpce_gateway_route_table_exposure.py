...
class TestDynamodbVpceGatewayRouteTableExposure(AwsBaseRuleTest):
    ...
    def test_invalid_dynamodb_vpce_association(self):  # table + eni + vpce + mismatch vpce
        self.run_test_case('invalid-dynamodb-vpce-association', True, always_use_cache_on_jenkins=True)

    def test_no_tables_in_region(self):  # eni + vpce + vpce route
        self.run_test_case('no-tables-in-region', False, always_use_cache_on_jenkins=True)

    def test_vpc_do_not_contains_any_eni(self):  # table + vpce + vpce route
        self.run_test_case('vpc-do-not-contains-any-eni', False, always_use_cache_on_jenkins=True)

    def test_dynamodb_vpce_exist_with_route_table_association(self):  # table + eni + vpce
        self.run_test_case('dynamodb-vpce-exist-with-route-table-association', False, always_use_cache_on_jenkins=True)

    def test_vpc_has_dynamodb_vpce_gw_valid_route(self):  # table + eni + vpce + vpce route
        self.run_test_case('vpc-has-dynamodb-vpce-gw-valid-route', False, always_use_cache_on_jenkins=True)

    def test_dynamodb_vpce_gw_not_exist_with_route(self):  # table + eni + vpce route
        self.run_test_case('dynamodb-vpce-gw-not-exist-with-route', False, always_use_cache_on_jenkins=True)
