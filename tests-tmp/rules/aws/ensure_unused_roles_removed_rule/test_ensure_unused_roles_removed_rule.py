...
class TestEnsureUnusedRolesRemoved(AwsBaseRuleTest):
    ...
    def test_new_role_lately_used(self):
        self.run_test_case('new_role_lately_used', False, use_state_file=True)

    @skip('skipped due to old data (more than 90 days')
    def test_new_role_never_used(self):
        self.run_test_case('new_role_never_used', False, use_state_file=True)

    @skip('skipped due to old data (more than 90 days')
    def test_old_role_used_lately(self):
        self.run_test_case('old_role_used_lately', False, use_state_file=True)

    @skip('skipped due to old data (more than 90 days')
    def test_old_role_not_used_lately(self):
        self.run_test_case('old_role_not_used_lately', True, use_state_file=True)

    @skip('skipped due to old data (more than 90 days')
    def test_old_role_not_used_at_all(self):
        self.run_test_case('old_role_not_used_at_all', True, use_state_file=True)
