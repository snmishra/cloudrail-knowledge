...
class TestEnsureKmsKeyPolicyNotUseWildcard(AwsBaseRuleTest):
    ...
    def test_kms_key_not_secure_policy(self):
        ...
        self.assertTrue('is using wildcard action `kms:*`, and principal `AWS: *`, without any condition' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'KMS key')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'KMS key resource policy')
