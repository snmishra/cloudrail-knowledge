...
class TestEnsureElasticSearchDomainPolicyNotUseWildcard(AwsBaseRuleTest):
    ...
    def test_not_secure_policy(self):
        ...
        self.assertTrue('is using wildcard action `es:*`, and principal `AWS: *`, without any condition' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'ElasticSearch Domain')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'ElasticSearch Domain resource policy')
