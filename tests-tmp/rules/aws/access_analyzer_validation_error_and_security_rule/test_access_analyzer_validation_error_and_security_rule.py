...
class TestAccessAnalyzerValidationErrorAndSecurityRule(AwsBaseRuleTest):
    ...
    def test_iam_multiple_issues(self):
        ...
        for issue_item in rule_result.issue_items:
            if issue_item.exposed.iac_entity_id == 'aws_iam_role_policy.allow-policy-1':
                self.assertIn('The action \'iam: passrole\' does not exist', issue_item.evidence)
            elif issue_item.exposed.iac_entity_id == 'aws_iam_role_policy.allow-policy-2':
                ...
                self.fail(f'issue item id {issue_item.exposed.iac_entity_id} is unknown')

    def test_iam_security_issues(self):
        rule_result = self.run_test_case('iam_security_issues', True, number_of_issue_items=1)
        evidence = rule_result.issue_items[0].evidence
