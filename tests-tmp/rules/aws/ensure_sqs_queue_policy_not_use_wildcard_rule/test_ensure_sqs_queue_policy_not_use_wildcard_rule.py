...
class TestEnsureSqsQueuePolicyNotUseWildcard(AwsBaseRuleTest):
    ...
    def test_bad_policy(self):
        ...
        self.assertTrue('is using wildcard action `sqs:*`, and principal `AWS: *`, without any condition' in rule_result.issue_items[0].evidence)
        self.assertEqual(rule_result.issue_items[0].exposed.type, 'SQS queue')
        self.assertEqual(rule_result.issue_items[0].violating.type, 'SQS queue resource policy')
