...
class TestS3VpceGatewayRouteTableExposure(AwsBaseRuleTest):
    ...
    def test_invalid_s3_vpce_association(self):  # bucket + eni + vpce + mismatch vpce
        self.run_test_case('invalid-s3-vpce-association', True, always_use_cache_on_jenkins=True)

    def test_no_bucket_in_region(self):  # igw + eni
        self.run_test_case('no-buckets-in-region', False, always_use_cache_on_jenkins=True)
    ...
    def test_vpce_exist_with_route_table_association(self):  # bucket + eni + vpce
        self.run_test_case('vpce-exist-with-route-table-association', False, always_use_cache_on_jenkins=True)

    def test_vpc_has_s3_vpce_gw_valid_route(self):  # bucket + eni + vpce + vpce route
        self.run_test_case('vpc-has-s3-vpce-gw-valid-route', False, always_use_cache_on_jenkins=True)

    def test_s3_vpce_gw_not_exist_with_route(self):  # bucket + eni + vpce route
        self.run_test_case('s3-vpce-gw-not-exist-with-route', False, always_use_cache_on_jenkins=True)
