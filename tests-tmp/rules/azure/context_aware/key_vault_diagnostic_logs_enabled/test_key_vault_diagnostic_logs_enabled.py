...
class TestKeyVaultDiagnosticLogsEnabledRule(AzureBaseRuleTest):
    ...
    def test_log_enabled_retention_disabled(self):
        ...
        self.assertTrue('have a disabled log retention policy' in rule_result.issue_items[0].evidence)

    def test_logs_not_defined(self):
        rule_result = self.run_test_case('logs_not_defined', True)
        self.assertIsNotNone(rule_result)
        self.assertTrue('does not have log block configuration' in rule_result.issue_items[0].evidence)

    def test_key_vault_no_monitoring(self):
        rule_result = self.run_test_case('key_vault_no_monitoring', True)
        self.assertIsNotNone(rule_result)
        self.assertTrue('does not have diagnostic settings' in rule_result.issue_items[0].evidence)

    def test_logs_configured_not_enabled(self):
        rule_result = self.run_test_case('logs_configured_not_enabled', True)
        self.assertIsNotNone(rule_result)
        self.assertTrue('does not have log enabled' in rule_result.issue_items[0].evidence)

    def test_no_retention_policy(self):
        rule_result = self.run_test_case('no_retention_policy', True)
        self.assertIsNotNone(rule_result)
        self.assertTrue('does not have a log retention policy' in rule_result.issue_items[0].evidence)
    ...
    def test_log_configureed_retention_days_violating(self):
        ...
        self.assertIsNotNone(rule_result)
        self.assertTrue('does not have a log retention policy days equal to 0 or greater than or equal to 365' in rule_result.issue_items[0].evidence)
