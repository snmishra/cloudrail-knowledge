...
class TestRds(AwsContextTest):
    ...
    def test_individual_defaults_only(self, ctx: AwsEnvironmentContext):
        ...
        self.assertFalse(ctx.rds_instances[0].encrypted_at_rest)
        self.assertEqual(ctx.rds_instances[0].backup_retention_period, 0)
    ...
    @context(module_path="aurora/cluster-with-instances", test_options=TestOptions(always_use_cache_on_jenkins=True))
