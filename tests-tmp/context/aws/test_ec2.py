...
from core.utils.policy_evaluator import PolicyEvaluation, is_any_action_allowed
from cloudrail.knowledge.context.connection import ConnectionDirectionType, PolicyConnectionProperty, PrivateConnectionDetail
from cloudrail.knowledge.context.aws.ec2.ec2_instance import AssociatePublicIpAddress
...
class TestEc2(AwsContextTest):
    ...
    @context(module_path="checkpoint_fw_cluster", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="defaults_only", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="in_subnet", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="using_common_module", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="conflict_between_subnet_and_ec2_public_settings", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="http_tokens_required", test_options=TestOptions(always_use_cache_on_jenkins=True))
    def test_http_tokens_required(self, ctx: AwsEnvironmentContext):
        ec2instance = ctx.ec2s[0]
        self.assertEqual(ec2instance.http_tokens, 'required')

    @context(module_path="using_gitlab_runner_module", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path='using_cloud42_bastion_module', test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="ec2_private_images_only", test_options=TestOptions(always_use_cache_on_jenkins=True))
    def test_ec2_private_images_only(self, ctx: AwsEnvironmentContext):
        for ec2 in ctx.ec2s:
            self.assertFalse(ec2.image_data.is_public)

    @context(module_path="ec2_3private_1public_image", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="ec2-outbound-permissions-connections")
    ...
    @context(module_path="no_tags", test_options=TestOptions(always_use_cache_on_jenkins=True))
    def test_no_tags(self, ctx: AwsEnvironmentContext):
        for ec2 in ctx.ec2s:
            self.assertFalse(ec2.tags)

    @context(module_path="monitoring_enabled", test_options=TestOptions(always_use_cache_on_jenkins=True))
    ...
    @context(module_path="monitoring_disabled", test_options=TestOptions(always_use_cache_on_jenkins=True))
