...
from core.utils.connection_utils import get_allowing_public_access_on_ports
...
class TestConnectionUtils(unittest.TestCase):
    ...
    def test_public_connection_exists_on_different_port(self):
        redshift = self._get_scenario_object('test_public_connection_exists')
        res = get_allowing_public_access_on_ports(redshift, [1111])
